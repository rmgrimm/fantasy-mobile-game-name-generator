val words = arrayOf(
    Word("Adventure"),
    Word("Adventurer"),
    Word("Age"),
    Word("Agent"),
    Word("Alliance"),
    Word("Amazon"),
    Word("Amulet"),
    Word("Angel"),
    Word("Anger"),
    Word("Apple"),
    Word("Archer"),
    Word("Arena"),
    Word("Armor"),
    Word("Arrival"),
    Word("Arrow"),
    Word("Assassin"),
    Word("Avalon"),
    Word("Ballista"),
    Word("Bandit"),
    Word("Bane"),
    Word("Banshee"),
    Word("Bard"),
    Word("Battle"),
    Word("Battlefield"),
    Word("Beginning"),
    Word("Beholder"),
    Word("Black", ""),
    Word("Blacksmith"),
    Word("Blade"),
    Word("Blessing"),
    Word("Blood"),
    Word("Bomb"),
    Word("Bond"),
    Word("Bone"),
    Word("Book"),
    Word("Bracer"),
    Word("Bridge"),
    Word("Buccaneer"),
    Word("Cabal"),
    Word("Call"),
    Word("Camelot"),
    Word("Cape"),
    Word("Castle"),
    Word("Catacomb"),
    Word("Catapult"),
    Word("Cauldron"),
    Word("Cave"),
    Word("Centaur"),
    Word("Chain"),
    Word("Chaos", "es"),
    Word("Chariot"),
    Word("Charm"),
    Word("Child", "ren"),
    Word("Chimera"),
    Word("Chronicle"),
    Word("Church", "es"),
    Word("Circle"),
    Word("Clan"),
    Word("Clash", "es"),
    Word("Cloud"),
    Word("Coast"),
    Word("Conquest"),
    Word("Contest"),
    Word("Coven"),
    Word("Covenant"),
    Word("Creed"),
    Word("Crest"),
    Word("Crown"),
    Word("Crusade"),
    Word("Cry", pluralOverride = "Cries"),
    Word("Crystal"),
    Word("Curse"),
    Word("Dagger"),
    Word("Darkness", "es"),
    Word("Defeat"),
    Word("Defender"),
    Word("Defense"),
    Word("Demon"),
    Word("Desire"),
    Word("Despair"),
    Word("Destiny", pluralOverride = "Destinies"),
    Word("Destroyer"),
    Word("Devil"),
    Word("Diamond"),
    Word("Djinn"),
    Word("Dominion"),
    Word("Dragon"),
    Word("Duel"),
    Word("Dungeon"),
    Word("Dust"),
    Word("Dynasty", pluralOverride = "Dynasties"),
    Word("Earth"),
    Word("Edge"),
    Word("Elf", pluralOverride = "Elves"),
    Word("Emerald"),
    Word("Emperor"),
    Word("Empire"),
    Word("End"),
    Word("Endless", ""),
    Word("Epic"),
    Word("Era"),
    Word("Eternal"),
    Word("Explosion"),
    Word("Eye"),
    Word("Fairy", pluralOverride = "Fairies"),
    Word("Fantasy", pluralOverride = "Fantasies"),
    Word("Fate"),
    Word("Feast"),
    Word("Fervor"),
    Word("Fever"),
    Word("Fight"),
    Word("Fire"),
    Word("Flame"),
    Word("Flesh", "es"),
    Word("Force"),
    Word("Forest"),
    Word("Fortune"),
    Word("Friend"),
    Word("Gambit"),
    Word("Galaxy", pluralOverride = "Galaxies"),
    Word("Game"),
    Word("Gate"),
    Word("Gathering"),
    Word("Gauntlet"),
    Word("Gear"),
    Word("Genie"),
    Word("Ghost"),
    Word("Gladiator"),
    Word("Gloom"),
    Word("Glory", pluralOverride = "Glories"),
    Word("Glyph"),
    Word("Goblet"),
    Word("Goblin"),
    Word("God"),
    Word("Goddess", "es"),
    Word("Gold"),
    Word("Golem"),
    Word("Grave"),
    Word("Groan"),
    Word("Grunt"),
    Word("Guardian"),
    Word("Guest"),
    Word("Gun"),
    Word("Gypsy", pluralOverride = "Gypsies"),
    Word("Harbinger"),
    Word("Heart"),
    Word("Hero", "es"),
    Word("Hole"),
    Word("Horde"),
    Word("Hunter"),
    Word("Hydra"),
    Word("Iron"),
    Word("Ice"),
    Word("Illusion"),
    Word("Immortal"),
    Word("Incubus", pluralOverride = "Incubi"),
    Word("Jewel"),
    Word("Joy"),
    Word("Keep"),
    Word("Key"),
    Word("King"),
    Word("Kingdom"),
    Word("Kiss", "es"),
    Word("Knight"),
    Word("Lady", pluralOverride = "Ladies"),
    Word("Lair"),
    Word("Lance"),
    Word("League"),
    Word("Legend"),
    Word("Legion"),
    Word("Lightning"),
    Word("Limit"),
    Word("Lion"),
    Word("Llama"),
    Word("Lord"),
    Word("Machination"),
    Word("Machine"),
    Word("Magic", "ks"),
    Word("Mask"),
    Word("Mercenary", pluralOverride = "Mercenaries"),
    Word("Mercy", pluralOverride = "Mercies"),
    Word("Mind"),
    Word("Moan"),
    Word("Moat"),
    Word("Mobile", ""),
    Word("Monster"),
    Word("Moon"),
    Word("Morning"),
    Word("Mystery", pluralOverride = "Mysteries"),
    Word("Myth"),
    Word("Mythology", pluralOverride = "Mythologies"),
    Word("Night"),
    Word("Noble"),
    Word("Nomad"),
    Word("Oath"),
    Word("Ogre"),
    Word("Omen"),
    Word("Oracle"),
    Word("Orb"),
    Word("Orc"),
    Word("Order"),
    Word("Origin"),
    Word("Overlord"),
    Word("Paladin"),
    Word("Pegasus", pluralOverride = "Pegasi"),
    Word("Phantom"),
    Word("Pharaoh"),
    Word("Phoenix", "es"),
    Word("Pirate"),
    Word("Pit"),
    Word("Plague"),
    Word("Plot"),
    Word("Poem"),
    Word("Poison"),
    Word("Portal"),
    Word("Potion"),
    Word("Prayer"),
    Word("Priest"),
    Word("Priestess", "es"),
    Word("Prince"),
    Word("Princess", "es"),
    Word("Prison"),
    Word("Prize"),
    Word("Promise"),
    Word("Prophecy", pluralOverride = "Prophecies"),
    Word("Quest"),
    Word("Quiver"),
    Word("Raid"),
    Word("Rain"),
    Word("Realm"),
    Word("Reign"),
    Word("Rifle"),
    Word("Rift"),
    Word("Ring"),
    Word("Rise"),
    Word("Road"),
    Word("Rogue"),
    Word("Ronin"),
    Word("Rose"),
    Word("Ruby", pluralOverride = "Rubies"),
    Word("Ruin"),
    Word("Rule"),
    Word("Rune"),
    Word("Sacrifice"),
    Word("Sage"),
    Word("Saint"),
    Word("Samurai"),
    Word("Savior"),
    Word("Scroll"),
    Word("Sea"),
    Word("Seven", ""),
    Word("Sewer"),
    Word("Shadow"),
    Word("Shield"),
    Word("Shrine"),
    Word("Siege"),
    Word("Sin"),
    Word("Siren"),
    Word("Skill"),
    Word("Skull"),
    Word("Sky", pluralOverride = "Skies"),
    Word("Snake"),
    Word("Snow"),
    Word("Soldier"),
    Word("Son"),
    Word("Song"),
    Word("Soul"),
    Word("Spell"),
    Word("Spike"),
    Word("Squire"),
    Word("Stalker"),
    Word("Star"),
    Word("Statue"),
    Word("Steel"),
    Word("Stone"),
    Word("Storm"),
    Word("Strategy", pluralOverride = "Strategies"),
    Word("Succubus", pluralOverride = "Succubi"),
    Word("Sultan"),
    Word("Summoner"),
    Word("Sun"),
    Word("Sword"),
    Word("Tactic"),
    Word("Tale"),
    Word("Temple"),
    Word("Tendril"),
    Word("Thief", pluralOverride = "Thieves"),
    Word("Thirteen", ""),
    Word("Thought"),
    Word("Thrall"),
    Word("Throne"),
    Word("Thunder"),
    Word("Tiara"),
    Word("Time"),
    Word("Titan"),
    Word("Tomb"),
    Word("Tome"),
    Word("Tower"),
    Word("Treasure"),
    Word("Trial"),
    Word("Tribe"),
    Word("Troll"),
    Word("Twin"),
    Word("Unicorn"),
    Word("Universe"),
    Word("Vagabond"),
    Word("Valkyrie"),
    Word("Victor"),
    Word("Victory", pluralOverride = "Victories"),
    Word("Void"),
    Word("Wall"),
    Word("War"),
    Word("Warlock"),
    Word("Warrior"),
    Word("Wheel"),
    Word("Wisdom"),
    Word("Wife", pluralOverride = "Wives"),
    Word("Witch", "es"),
    Word("Wizard"),
    Word("Wolf", pluralOverride = "Wolves"),
    Word("Wonder"),
    Word("World"),
    Word("Worm"),
    Word("Wyvern"),

    // Modifiers
    Word("Dark {s}", pluralOverride = "Dark {p}"),
    Word("Enchanted {s}", pluralOverride = "Enchanted {p}"),
    Word("Evil {s}", pluralOverride = "Evil {p}"),
    Word("Fallen {s}", pluralOverride = "Fallen {p}"),
    Word("Final {s}", pluralOverride = "Final {p}"),
    Word("Forgotten {s}", pluralOverride = "Forgotten {p}"),
    Word("Goblin {s}", pluralOverride = "Goblin {p}"),
    Word("Good {s}", pluralOverride = "Good {p}"),
    Word("Heartless {s}", pluralOverride = "Heartless {p}"),
    Word("Last {s}", pluralOverride = "Last {p}"),
    Word("Lost {s}", pluralOverride = "Lost {p}"),
    Word("Orcish {s}", pluralOverride = "Orcish {p}"),
    Word("Wooden {s}", pluralOverride = "Wooden {p}"),
    Word("{s}breaker"),
    Word("{s}-hearted"),
    Word("{s}-hunter"),
    Word("{s}keeper"),
    Word("{s}rider"),
    Word("{s}-seeker"),
    Word("Dark{sl}", pluralOverride = "Dark{pl}"),
    Word("Wild{sl}", pluralOverride = "Wild{pl}"),
    Word("{s}{sl}", pluralOverride = "{s}{pl}")
)
